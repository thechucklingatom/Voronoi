...
public class MapsAPIAccesor {
	private final String API_KEY = "AIzaSyBpr_kg5qcrWlylWC8yDsOjEaQRyLF8Pjc";
	...
	/**
	 * Gets the image with the places marked on it.
	 * @param places The list of the places that you want to add
	 * @return The {@link Image} that was returned by the api.
	 */
	public Image getMapImageWithExtras(List<PlacesLocation> places) {
		try {
			String listOfPlaces = "";

			for(PlacesLocation place : places){
				listOfPlaces += place.getGeometry().getLocation().getLat() + ","
						+ place.getGeometry().getLocation().getLng() + "|";
			}

			listOfPlaces = listOfPlaces.substring(0, listOfPlaces.length() - 2);
			return ImageIO.read(new URL(MAP_IMAGE_ENDPOINT
					+ "markers="
					+ listOfPlaces
